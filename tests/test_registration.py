...
def test_success_registration(driver):
    ...
    # Act
    driver.get(REG_URL)
    WebDriverWait(driver, 100).until(expected_conditions.visibility_of_element_located(REG_BUTTON))
    ...
    # Assert
    assert WebDriverWait(driver, 100).until(expected_conditions.url_contains(LOGIN_URL))
...
def test_error_short_password(driver):
    ...
    # Assert
    error_element = WebDriverWait(driver, 100).until(expected_conditions.visibility_of_element_located(REG_ERROR))
